...
from app.xianyu_adapter.http_client import XianyuApiClient
from app.xianyu_adapter.utils import generate_mid, generate_uuid, trans_cookies, generate_device_id, decrypt
...
class XianyuLiveClient:
    ...
    处理与闲鱼服务器的WebSocket通信，监听并响应用户消息。
    """
    ...
    def __init__(self, cookies_str):
        ...
        """
        self.xianyu_api = XianyuApiClient()
        self.base_url = 'wss://wss-goofish.dingtalk.com/'
        self.cookies_str = cookies_str
        self.cookies = trans_cookies(cookies_str)
        self.xianyu_api.session.cookies.update(self.cookies)  # 直接使用 session.cookies.update
    ...
    async def init(self, ws):
        ...
        Args:
            ws: WebSocket连接
        """
        token = self.xianyu_api.get_token(self.device_id)['data']['accessToken']
        msg = {
            "lwp": "/reg",
            "headers": {
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            item_info = self.conversation_service.get_item_info(item_id)
            if not item_info:
                ...
                if 'data' in api_result and 'itemDO' in api_result['data']:
                    ...
                    # 保存商品信息到数据库
                    self.conversation_service.save_item_info(item_id, item_info)
                else:
            ...
            # 添加用户消息到上下文
            self.conversation_service.add_message(send_user_id, item_id, "user", send_message)
            
            # 获取完整的对话上下文
            context = self.conversation_service.get_context(send_user_id, item_id)
            ...
            if self.reply_bot.last_intent == "price":
                self.conversation_service.increment_bargain_count(send_user_id, item_id)
                bargain_count = self.conversation_service.get_bargain_count(send_user_id, item_id)
                logger.info(f"用户 {send_user_name} 对商品 {item_id} 的议价次数: {bargain_count}")
            
            # 添加机器人回复到上下文
            self.conversation_service.add_message(send_user_id, item_id, "assistant", bot_reply)
